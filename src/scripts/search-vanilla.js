...
function initSearch() {
  const A_SearchInput = document.querySelector(".A_Placeholder_Search");
  const A_SearchButton = document.querySelector(".A_Icon_Search");
...
function searchContent(requestText) {
  ...
  content.forEach((contentDataItem) => {
    ...
    if (requestText.length >= 3) {
      if (
        title.toLowerCase().includes(requestText.toLowerCase()) ||
...
document.addEventListener("DOMContentLoaded", () => {
  const form = document.querySelector(".M_SearchBox");
